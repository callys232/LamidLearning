...
import { streamChatResponse, getActiveProvider, getActiveModel, PROVIDER_META } from "@/lib/ai-providers";
...
export async function POST(req: NextRequest) {
  try {
    const { messages, context }: { messages: ChatMessage[]; context: TutorContext } =
      await req.json();
    ...
    const stream = await streamChatResponse(messages, buildSystemPrompt(context));

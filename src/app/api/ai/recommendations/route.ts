...
export async function POST(req: NextRequest) {
  try {
    const { enrolledCourseIds, completedCourseIds, quizAvgScore, categoryFocus, availableCourses } =
      await req.json();
    ...
    const courseList = availableCourses
      .map((c: { id: string; title: string; categoryId: string; difficulty: string }) =>
...
Only include courses from the list above.`;
    ...
    const recommendations: RecommendedCourse[] = match ? JSON.parse(match[0]) : [];

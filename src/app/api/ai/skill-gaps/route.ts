...
import { generateText, isAiKeyMissing } from "@/lib/ai-providers";
...
export async function POST(req: NextRequest) {
  try {
    const { progress, quizAttempts, completedCourses, enrolledCourses, availableCourses } =
      await req.json();

    if (isAiKeyMissing()) {
      const fallback: SkillGapResult = {
        gaps: [
          { area: "Prototyping",       description: "No prototyping courses completed yet.",   severity: "high",   courseId: "course-003" },
          { area: "Software Dev",      description: "Limited technical module coverage.",       severity: "medium", courseId: "course-004" },
          { area: "Business Strategy", description: "No business strategy content attempted.", severity: "low",    courseId: "course-005" },
        ],
        strengths:   ["HCD Foundations", "UX Research", "Consistent learning streak"],
        suggestions: [
          { action: "Complete Prototyping & Validation to close the top gap", courseId: "course-003" },
          { action: "Start Modern React Development for technical breadth",    courseId: "course-004" },
        ],
        overallScore: 42,
      };
    ...
    const correct = quizAttempts.filter((a: { isCorrect: boolean }) => a.isCorrect).length;
    const quizScore = quizAttempts.length > 0
      ? Math.round((correct / quizAttempts.length) * 100)
      : null;

    const courseMap = availableCourses
      .map((c: { id: string; title: string; categoryId: string }) => `${c.id}: "${c.title}" (${c.categoryId})`)
      .join("\n");
...
{"gaps":[{"area":"...","description":"...","severity":"low|medium|high","courseId":"optional"}],"strengths":["..."],"suggestions":[{"action":"...","courseId":"optional"}],"overallScore":0-100}`;
    ...
    return NextResponse.json({ gaps: [], strengths: [], suggestions: [], overallScore: 0 });
  }
}

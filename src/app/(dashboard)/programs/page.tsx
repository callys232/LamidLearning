...
export default function ProgramsPage() {
  ...
  return (
    <div>
      <Header title="Programs" subtitle="Multi-course learning tracks with certification" user={currentUser} />
      ...
      <div className="px-6 py-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {mockPrograms.map((program) => {
            ...
            return (
              <Card key={program.id} className="group hover:border-primary/30 transition-all">
                <CardContent className="p-6">
                  {/* Header */}
                  <div className="mb-4 flex items-start justify-between gap-3">
                    <div>
                      {category && (
                        <Badge variant={categoryVariant[program.categoryId] ?? "default"} className="mb-2">
                          {category.code}
                        </Badge>
                      )}
                  ...
                  <p className="mb-4 text-sm text-text-secondary leading-relaxed">{program.description}</p>
                  ...
                  <div className="mb-5 flex flex-col gap-1.5">
                    {courses.map((course, i) => (
                      <div key={course.id} className="flex items-center gap-2 text-xs text-text-secondary">

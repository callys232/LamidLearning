...
export default function EventsPage() {
  ...
  const recordedEvents  = mockEvents.filter((e) => e.status === "completed" && e.recordingUrl);
  const totalRegistered = new Set(mockEvents.flatMap((e) => e.registeredUserIds)).size;
  ...
  const stats = [
    { icon: Calendar,    label: "Upcoming Sessions",  value: upcomingEvents.length,  color: "text-blue-400"    },
    { icon: Radio,       label: "Live Right Now",      value: liveEvents.length,      color: "text-red-400"     },
    { icon: PlayCircle,  label: "Recordings Available",value: recordedEvents.length,  color: "text-violet-400"  },
    { icon: Users,       label: "Total Learners",      value: formatNumber(mockUsers.length), color: "text-emerald-400"},
  ];
      ...
      <div className="px-6 py-6 space-y-6">
          ...
          {liveEvents.length > 0 && (
            <div className="flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/8 px-3 py-1.5">
              ...
              <span className="text-xs font-semibold text-red-400">
                {liveEvents.length} Session{liveEvents.length > 1 ? "s" : ""} Live Now
              </span>
            </div>
          )}
        </div>
        ...
        {sorted.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-medium text-text-primary">No sessions here</p>
            <p className="text-xs text-text-muted mt-1">Try switching to a different filter.</p>
          </div>
        ) : (

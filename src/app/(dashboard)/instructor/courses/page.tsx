...
export default function InstructorCoursesPage() {
  ...
  const myCourses = hasAccess ? mockCourses.filter((c) => c.instructorId === user.id) : [];

  return (
    <div>
      <Header
        title="My Courses"
        subtitle={hasAccess ? "Manage your course library" : "Access restricted"}
        user={user}
      />
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="mt-2 text-sm text-text-secondary">
                Learners can attend events and courses, but only instructors and admins can manage course content.
              </p>
            </CardContent>
          </Card>
        ) : (
            ...
            {myCourses.length === 0 ? (
              <Card>
                <CardContent className="p-10 text-center">
                  ...
                  <Link href="/instructor/courses/new" className="mt-3 inline-block text-sm text-primary hover:underline">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border text-left">
                        ...
                        <th className="px-5 py-3 text-xs font-medium text-text-muted hidden sm:table-cell">Enrolled</th>
                        <th className="px-5 py-3 text-xs font-medium text-text-muted hidden md:table-cell">Rating</th>
                    ...
                    <tbody className="divide-y divide-border">
                      {myCourses.map((course) => (
                        <tr key={course.id} className="group hover:bg-surface transition-colors">
                          <td className="px-5 py-3 max-w-xs">
                            <p className="font-medium text-text-primary line-clamp-1">{course.title}</p>
                            <p className="text-xs text-text-muted capitalize">{course.difficulty} · {course.estimatedHours}h</p>
                          </td>
                          <td className="px-5 py-3 hidden sm:table-cell text-text-secondary">
                            {formatNumber(course.enrollmentCount)}
                          </td>
                          <td className="px-5 py-3 hidden md:table-cell">
                            <span className="text-xs font-medium text-yellow-400">★ {course.rating}</span>
                          </td>

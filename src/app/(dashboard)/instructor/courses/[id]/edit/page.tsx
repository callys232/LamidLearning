...
import { ArrowLeft, BookOpen, Layers, HelpCircle, Eye, Save } from "lucide-react";
import Link from "next/link";
import { Header } from "@/components/layout";
import { Card, CardContent, Input, Button, Badge, Separator } from "@/components/ui";
import {
  ModuleBuilder,
  QuizBuilder,
  type ModuleDraft,
...
import type { DifficultyLevel, AccessType, ContentStatus, LessonType } from "@/types";
      ...
      <div>
        ...
        <div className="px-6 py-10 text-center">
          <p className="text-sm text-text-muted">
            No course with ID <code className="font-mono text-xs bg-surface px-1 py-0.5 rounded">{id}</code> was found.{" "}
      ...
      <div className="px-6 py-6 max-w-3xl">
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-2">
                ...
                {i < STEPS.length - 1 && (
                  <div className={cn("h-px w-6 transition-colors", i < stepIdx ? "bg-emerald-500/40" : "bg-border")} />
                )}
              </div>
            );
          })}
          ...
          <Card>
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="edit-course-description" className="text-sm font-medium text-text-primary">
                  Description
                </label>
                <textarea
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="edit-course-category" className="text-sm font-medium text-text-primary">Category</label>
                  <select
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="edit-course-difficulty" className="text-sm font-medium text-text-primary">Difficulty</label>
                  <select
                    id="edit-course-difficulty"
                    value={form.difficulty}
                    onChange={(e) => updateForm({ difficulty: e.target.value as DifficultyLevel })}
                    className="rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-primary/50"
                  >
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="edit-course-access" className="text-sm font-medium text-text-primary">Access</label>
                  <select
                    id="edit-course-access"
                    value={form.accessType}
                    onChange={(e) => updateForm({ accessType: e.target.value as AccessType })}
                    className="rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-primary/50"
                  >
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  {
                    label: "Category",
                    value: mockServiceCategories.find((c) => c.id === form.categoryId)?.label,
                  },
                  { label: "Difficulty", value: form.difficulty },
                  {
                    label: "Access",
                    value: form.accessType + (form.price ? ` · $${form.price}` : ""),
                  },
                  {
                    label: "Modules",
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          {stepIdx < STEPS.length - 1 && (
            <Button variant="primary" onClick={() => setStep(STEPS[stepIdx + 1].id)}>
              Next →
            </Button>
          )}
        </div>

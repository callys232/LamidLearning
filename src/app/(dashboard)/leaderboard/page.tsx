...
const LEVELS = [
  { level: 1, name: "Novice",   min: 0,      max: 499,      dot: "bg-slate-400",   badge: "bg-slate-500/20 text-slate-300"    },
  { level: 2, name: "Explorer", min: 500,    max: 1_499,    dot: "bg-blue-400",    badge: "bg-blue-500/20 text-blue-300"      },
  { level: 3, name: "Learner",  min: 1_500,  max: 2_999,    dot: "bg-emerald-400", badge: "bg-emerald-500/20 text-emerald-300"},
  { level: 4, name: "Scholar",  min: 3_000,  max: 5_999,    dot: "bg-violet-400",  badge: "bg-violet-500/20 text-violet-300"  },
  { level: 5, name: "Expert",   min: 6_000,  max: 11_999,   dot: "bg-yellow-400",  badge: "bg-yellow-500/20 text-yellow-300"  },
  { level: 6, name: "Master",   min: 12_000, max: 24_999,   dot: "bg-orange-400",  badge: "bg-orange-500/20 text-orange-300"  },
  { level: 7, name: "Champion", min: 25_000, max: 49_999,   dot: "bg-red-400",     badge: "bg-red-500/20 text-red-300"        },
  { level: 8, name: "Legend",   min: 50_000, max: Infinity, dot: "bg-text-primary",badge: "bg-surface-active text-text-primary"},
] as const;
...
export default function LeaderboardPage() {
  ...
  const podiumStyles = [
    ...
    { bg: "bg-yellow-400/10 border-yellow-400/30", ring: "ring-1 ring-yellow-400/40", height: "pt-0"  },
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      style.bg, style.ring, style.height,
                    )}
                  >
                    <span className="text-2xl leading-none" aria-label={`Rank ${user.rank}`}>
                      {medals[user.rank - 1]}
                    </span>
                    <div
                      className={cn(
                    ...
                    <p className={cn("text-sm font-bold tabular-nums", isFirst ? "text-yellow-400" : "text-primary")}>
                      {formatNumber(user.xp)} XP
                    </p>
                    <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold", lvl.badge)}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 w-10 text-center">
                            <span className="text-sm font-bold tabular-nums text-text-muted">
                              {user.rank <= 3
                                ? <span aria-label={`Rank ${user.rank}`}>{medals[user.rank - 1]}</span>
                                : user.rank}
                            </span>
                          </td>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-1.5">
                                  ...
                                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold shrink-0", lvl.badge)}>
                                    Lv.{lvl.level} {lvl.name}
                                  </span>
                                </div>
                                {user.headline && (
                          ...
                          <td className="px-4 py-3 hidden sm:table-cell text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <span className="text-xs text-text-secondary tabular-nums">{user.streak}d</span>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <p className={cn(
                              ...
                              user.rank === 1 ? "text-yellow-400" : "text-primary",
                            )}>
              ...
              <CardContent className="space-y-2.5 pb-4">
                {LEVELS.map((lvl) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn("h-2.5 w-2.5 rounded-full shrink-0", lvl.dot)} />
                      <span className="text-xs font-bold text-text-muted w-7 shrink-0">
                        Lv.{lvl.level}
                      </span>
                      <span className={cn("text-xs flex-1 font-medium", isMine ? "text-primary" : "text-text-primary")}>
                        {lvl.name}
                      </span>
              ...
              <CardContent className="space-y-2.5 pb-4">
                {XP_REWARDS.map(({ action, xp }) => (
                  <div key={action} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs font-bold text-primary shrink-0">+{xp} XP</span>
                  </div>
                ))}
              </CardContent>
            </Card>

...
export default function CoursesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (price === "free") courses = courses.filter((c) => c.accessType === "free");
    if (price === "paid") courses = courses.filter((c) => c.accessType !== "free");

    switch (sort) {
      case "popular":    return courses.sort((a, b) => b.enrollmentCount - a.enrollmentCount);
      case "rating":     return courses.sort((a, b) => b.rating - a.rating);
      case "newest":     return courses.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      case "price-asc":  return courses.sort((a, b) => (a.price ?? 0) - (b.price ?? 0));
      case "price-desc": return courses.sort((a, b) => (b.price ?? 0) - (a.price ?? 0));
  ...
  const featuredCourses = useMemo(
    () => [...mockCourses].sort((a, b) => b.enrollmentCount - a.enrollmentCount).slice(0, 3),
    [],
  );

  return (
    <div>
      <Header title="Courses" subtitle="Browse and continue your learning" user={currentUser} />
        ...
        <div className="flex gap-6 items-start">
          ...
          <aside className="hidden lg:flex w-52 shrink-0 flex-col gap-6">
            ...
            <div>
              ...
              <SidebarFilter label="All Categories" active={!category} onClick={() => setCategory(null)} />
            ...
            <div>
              ...
              <SidebarFilter label="All Levels" active={!level} onClick={() => setLevel(null)} />
              {LEVELS.map((lvl) => (
                <SidebarFilter
                  key={lvl}
            ...
            <div>
              ...
              <SidebarFilter label="All Prices" active={price === "all"} onClick={() => setPrice("all")} />
              <SidebarFilter label="Free"       active={price === "free"} onClick={() => setPrice("free")} />
              <SidebarFilter label="Paid"       active={price === "paid"} onClick={() => setPrice("paid")} />
            </div>
          </aside>

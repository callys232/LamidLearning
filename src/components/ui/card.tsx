  ...
  ({ className, hoverable, lift, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        hoverable && "cursor-pointer hover:border-primary/25 hover:shadow-soft-md transition-all duration-200",
        lift && liftClass[lift],
        className
      )}
      {...props}
...
const CardTitle = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-base font-semibold text-text-primary leading-none", className)} {...props} />
  )
);

...
export function CourseRecommendations({
  ...
  const availableCourses = mockCourses.filter(
    (c) => !enrolledCourseIds.includes(c.id) && !completedCourseIds.includes(c.id)
  );
  ...
  useEffect(() => {
    if (!availableCourses.length) { setLoading(false); return; }
    setLoading(true);
    ...
    fetch("/api/ai/recommendations", {
      ...
      body: JSON.stringify({
        ...
        availableCourses: availableCourses.map((c) => ({
          id: c.id, title: c.title, categoryId: c.categoryId, difficulty: c.difficulty,
        })),
      }),
    })
      .then((r) => r.json())
    ...
    <Card className="border-primary/15">
      ...
      <CardContent className="px-0 pb-0">
        {loading ? (
          ...
          <p className="px-5 pb-5 text-xs text-text-muted">No recommendations available — keep learning to unlock personalised suggestions.</p>
        ) : (
          <ul className="divide-y divide-border">
            {recs.map((rec) => {
              ...
              return (
                <RecommendationRow key={rec.courseId} course={course} rec={rec} />
              );
            })}
          </ul>
        )}
...
function RecommendationRow({ course, rec }: { course: Course; rec: RecommendedCourse }) {
  return (
      ...
      <Link href={`/courses/${course.id}`} className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1.5 flex items-center gap-3 text-[10px] text-text-muted">
            <span className="flex items-center gap-1"><Clock className="h-2.5 w-2.5" />{course.estimatedHours}h</span>
            <span className="flex items-center gap-1"><Star className="h-2.5 w-2.5 fill-yellow-400 text-yellow-400" />{course.rating}</span>
            <span className={cn(
              "ml-auto font-medium",
              rec.priority === "high" ? "text-primary" : rec.priority === "medium" ? "text-yellow-400" : "text-text-muted"
            )}>

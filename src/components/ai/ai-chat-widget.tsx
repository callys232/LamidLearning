...
function RobotBook({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      ...
      {/* Arms */}
      <line x1="7" y1="15.5" x2="8" y2="15.5" />
      <line x1="16" y1="15.5" x2="17" y2="15.5" />
      {/* Legs */}
...
function getWelcome(ctx: PageContext): string {
  ...
  const pageMap: Record<string, string> = {
    analytics:    "I can help you interpret your learning analytics and identify areas to improve.",
    events:       "I can help you decide which lectures to attend and how they fit your learning path.",
    certificates: "I can explain your certifications and suggest paths toward your next one.",
    instructor:   "I can help you design better courses and understand your learner engagement data.",
    dashboard:    "I'm here to help with anything on the LAMID platform — courses, progress, or learning advice.",
  };
...
function saveMessages(page: string, lesson: string | undefined, msgs: Message[]) {
  try {
...
export function AiChatWidget() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setMessages([{ id: "welcome", role: "assistant", content: getWelcome(ctx) }]);
    }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => saveMessages(ctx.page, ctx.lessonTitle, messages), 500);
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantId
            ? { ...m, content: "Sorry, I couldn't connect right now. Try again.", streaming: false }
            : m
        )
      );
    }
  ...
  return (
    <>
      ...
      <div className={cn("fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3", open && "pointer-events-none opacity-0 transition-opacity duration-200")}>
        {/* Tooltip */}
        <div className="group relative">
          ...
          >
            ...
            <RobotBook className="relative h-6 w-6" />
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-border bg-background-tertiary px-4 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative flex h-8 w-8 items-center justify-center rounded-full bg-primary shadow-primary-sm">
              <RobotBook className="h-4 w-4 text-white" />
            ...
            <div className="leading-none">
              <p className="text-sm font-semibold text-text-primary">LAMID AI</p>
              <p className="mt-0.5 text-[10px] text-text-muted">Your learning assistant</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
            {/* Context badge */}
            {ctx.page !== "dashboard" && (
              <Badge variant="primary" className="text-[9px] mr-1 capitalize">{ctx.page}</Badge>
            )}
            <button
              type="button"
              onClick={() => setMessages([{ id: "welcome", role: "assistant", content: getWelcome(ctx) }])}
            ...
            <div key={msg.id} className={cn("flex gap-2", msg.role === "user" ? "justify-end" : "justify-start")}>
              {msg.role === "assistant" && (
                <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-muted">
                  <RobotBook className="h-3.5 w-3.5 text-primary" />
                </div>
              )}
              <div
                className={cn(
        ...
        <div className="shrink-0 border-t border-border p-3">
          <div className="flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 focus-within:border-primary/50 focus-within:shadow-[0_0_0_3px_rgba(193,33,41,0.08)] transition-all duration-150">
            <input
              ...
              placeholder={ctx.lessonTitle ? `Ask about ${ctx.lessonTitle}…` : "Ask anything…"}
              disabled={loading}

...
import type { Course, Module, Lesson, LearnerProgress } from "@/types";
...
function getNextLesson(modules: Module[], lessons: Lesson[], completedLessonIds: string[]): Lesson | undefined {
...
export function CourseProgressPanel({
  ...
  const nextLesson = getNextLesson(modules, lessons, progress.completedLessonIds);
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-semibold text-text-primary">{progress.percentComplete}%</span>
        </div>
        <Progress value={progress.percentComplete} />
        <p className="text-xs text-text-muted">
          {remaining > 0 ? `${remaining} remaining · ~${formatMinutes(remainingMinutes)} left` : "Course complete 🎉"}
        </p>
      </div>
    );
  }
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="shrink-0 text-xl font-bold text-primary">{progress.percentComplete}%</span>
        </div>
        <div className="mt-2 flex flex-col gap-1.5">
          <Progress value={progress.percentComplete} barClassName="bg-primary" />
          <p className="text-xs text-text-muted">
            {doneLessons} of {totalLessons} lessons done
            {remaining > 0 && ` · ${remaining} remaining · ~${formatMinutes(remainingMinutes)} left`}
      ...
      <CardContent className="px-0 pb-0">
        ...
        <ul className="divide-y divide-border">
          {modules.map((mod) => {
            const status = getModuleStatus(mod, progress.completedModuleIds, progress.completedLessonIds);
            const modDone = mod.lessonIds.filter((id) => progress.completedLessonIds.includes(id)).length;

            return (
              <li key={mod.id} className="flex items-center gap-3 px-5 py-3">
                <ModuleStatusIcon status={status} />
                <div className="flex flex-1 flex-col gap-0.5 min-w-0">
                  <p className={cn("truncate text-sm font-medium", status === "locked" ? "text-text-muted" : "text-text-primary")}>
                    {mod.title}
                  </p>
                  <p className="text-xs text-text-muted">
                    {status === "locked"
                ...
                {status === "in_progress" && (
                  <div className="w-16 shrink-0">
                    <Progress value={Math.round((modDone / mod.lessonIds.length) * 100)} />
                  </div>
                )}
              </li>
            );
...
function ModuleStatusIcon({ status }: { status: ReturnType<typeof getModuleStatus> }) {
  if (status === "completed") return <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-400" />;
  if (status === "in_progress") return <PlayCircle className="h-4 w-4 shrink-0 text-primary" />;
  if (status === "locked") return <Lock className="h-4 w-4 shrink-0 text-text-muted" />;

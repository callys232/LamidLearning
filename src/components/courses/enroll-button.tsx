...
export function EnrollButton({ course, isEnrolled: initialEnrolled, firstLessonId }: EnrollButtonProps) {
  ...
  async function handleEnroll() {
    ...
    setLoading(false);
    setTimeout(() => {
      if (firstLessonId) router.push(`/courses/${course.id}/learn/${firstLessonId}`);
    }, 900);
  }
  ...
  return (
    ...
    >
      {loading ? (
        <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Enrolling…</span>
      ) : justDone ? (
        <span className="flex items-center justify-center gap-2"><CheckCircle2 className="h-4 w-4" /> Enrolled! Starting…</span>
      ) : enrolled ? (
        "Continue Learning →"

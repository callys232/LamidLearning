...
export function ContentGate({ course, isEnrolled, children }: ContentGateProps) {
...
function PaywallCard({ course }: { course: Course }) {
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 py-16 text-center animate-fade-in">
      ...
      <h2 className="mb-2 text-xl font-bold text-text-primary">Premium Content</h2>
      <p className="mb-2 text-sm text-text-secondary max-w-sm">
        This lesson is part of <span className="font-semibold text-text-primary">{course.title}</span>, a paid course.
        Enroll to unlock all {course.moduleIds.length} modules, quizzes, and your certificate.
      </p>
      ...
      <div className="flex flex-col items-center gap-3 w-full max-w-xs">
        ...
        <Link href="/courses" className="text-xs text-text-muted hover:text-primary transition-colors">
      ...
      <div className="mt-8 flex items-center gap-2 rounded-full border border-border bg-surface px-4 py-2">
        ...
        <p className="text-xs text-text-secondary">First module preview available — <Link href={`/courses/${course.id}`} className="text-primary hover:underline">view course</Link></p>
      </div>
    </div>
  );
}
...
export function LockedModuleGate({ moduleTitle, releaseDate }: LockedModuleGateProps) {
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center px-6 py-16 text-center animate-fade-in">
      ...
      <h2 className="mb-2 text-lg font-bold text-text-primary">Module Locked</h2>
      <p className="text-sm text-text-secondary max-w-sm">
        <span className="font-medium text-text-primary">{moduleTitle}</span> will unlock{" "}
        {releaseDate

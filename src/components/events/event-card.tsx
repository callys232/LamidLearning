...
import {
  Clock, Users, Video, Radio, PlayCircle,
  Calendar, Bookmark, BookmarkCheck,
} from "lucide-react";
import { cn, formatMinutes } from "@/lib/utils";
import type { EventFormat, LearningEvent, Course } from "@/types";
...
const FORMAT_CONFIG: Record<EventFormat, { icon: React.ElementType; label: string }> = {
...
export function EventCard({ event, course, userId, className }: EventCardProps) {
  ...
  const isRegistered  = userId ? event.registeredUserIds.includes(userId)  : false;
  const attended      = userId ? event.attendedUserIds.includes(userId)     : false;
  const watchedReplay = userId ? event.replayWatchedByUserIds.includes(userId) : false;
  ...
  return (
    <div
      className={cn(
        ...
        cfg.card,
        !cfg.card && "border-border hover:border-border-strong hover:shadow-soft-md",
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mb-3 flex items-start justify-between gap-2">
          <span className={cn(
            ...
            <span className={cn("h-1.5 w-1.5 rounded-full", cfg.dot, isLive && "animate-pulse")} />
            {statusLabel}
          </span>
        ...
        <h3 className={cn(
          ...
          isLive ? "text-primary" : "text-text-primary group-hover:text-primary",
        )}>
          {event.title}
        </h3>
        ...
        <div className="mb-4 space-y-1.5">
          ...
          {course && (
            <p className="text-[10px] text-text-muted truncate">{course.title}</p>
          )}
        </div>

...
export function CourseCard({ course, progress, isEnrolled, className }: CourseCardProps) {
    ...
    <Link href={`/courses/${course.id}`} className="block group">
      ...
      >
        ...
        <div className="relative h-44 w-full overflow-hidden bg-surface-active">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary-muted border border-primary/20 shadow-primary-sm group-hover:scale-110 group-hover:shadow-primary transition-all duration-300">
              <span className="text-2xl font-bold text-primary">{course.title[0]}</span>
            </div>
          </div>
          ...
          <div className="absolute left-3 top-3 flex items-center gap-1.5">
            {lvlCfg && (
              <span className={cn("rounded-full px-2.5 py-0.5 text-[10px] font-semibold text-white", lvlCfg.bg)}>
                {lvlCfg.label}
              </span>
            )}
            {discountPct && (
        ...
        <div className="p-4 space-y-2.5">
          {/* Category label */}
          {catCfg && (
            <p className={cn("text-[10px] font-bold uppercase tracking-widest", catCfg.color)}>
              {catCfg.label}
            </p>
          )}
          ...
          {instructor && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-text-secondary">{instructor.name}</span>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-text-muted">
            <span className="flex items-center gap-1 shrink-0">
              ...
              {course.ratingCount && (
                <span className="text-text-muted">({formatNumber(course.ratingCount)})</span>
              )}
            </span>
            <span className="flex items-center gap-1 shrink-0">
          ...
          <div className="flex items-end justify-between pt-1 border-t border-border">
            <div className="flex items-baseline gap-1.5">
              {course.accessType === "free" ? (
                ...
                <>
                  <span className="text-base font-bold text-text-primary">${course.price}</span>

...
import {
  ...
  Video, FileText, Presentation, Download,
} from "lucide-react";
...
let uid = 1000;
function nextId() { return `draft-${++uid}`; }
...
export function ModuleBuilder({ modules, onChange }: ModuleBuilderProps) {
  ...
  const [dragLesson, setDragLesson]               = useState<{ modId: string; lessonId: string } | null>(null);
  ...
  function addModule() {
    const m: ModuleDraft = { id: nextId(), title: "", description: "", lessons: [] };
    onChange([...modules, m]);
  ...
  function addLesson(modId: string) {
    const l: LessonDraft = { id: nextId(), title: "", type: "video", durationMinutes: 10 };
    updateModule(modId, {
  ...
  function removeLesson(modId: string, lessonId: string) {
    updateModule(modId, {
      lessons: modules.find((m) => m.id === modId)?.lessons.filter((l) => l.id !== lessonId) ?? [],
    });
  }
  ...
  function duplicateLesson(modId: string, lessonId: string) {
    ...
    if (!src || !mod) return;
    const copy = { ...src, id: nextId(), title: src.title ? `${src.title} (copy)` : "" };
  ...
  function updateLesson(modId: string, lessonId: string, patch: Partial<LessonDraft>) {
    updateModule(modId, {
      lessons:
        modules.find((m) => m.id === modId)?.lessons.map((l) =>
  ...
  function onLessonDragStart(e: React.DragEvent, modId: string, lessonId: string) {
    e.stopPropagation();
  ...
  function onLessonDrop(e: React.DragEvent, targetModId: string, targetLessonId: string) {
    e.preventDefault();
    e.stopPropagation();
    if (!dragLesson || dragLesson.lessonId === targetLessonId || dragLesson.modId !== targetModId) return;
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-3 bg-surface-hover">
              ...
              />
              <span className="text-xs text-text-muted shrink-0">{mod.lessons.length} lessons</span>
              <button
                type="button"
                ...
                {mod.lessons.map((lesson, li) => {
                  ...
                  return (
                    <div
                      key={lesson.id}
                      draggable
                      onDragStart={(e) => onLessonDragStart(e, mod.id, lesson.id)}
                      ...
                      className={cn(
                        ...
                        isLessonDragging ? "opacity-40 border-primary/40"          : "border-border",
                        isLessonDragOver ? "border-primary ring-1 ring-primary/30" : "",
                      )}
                    >
                      {/* Desktop: drag handle */}
                      ...
                      <span className="text-[10px] text-text-muted w-4 shrink-0 tabular-nums">{li + 1}</span>

                      {/* Lesson type icon toggles */}
                      <div className="flex gap-0.5 shrink-0" role="group" aria-label="Lesson type">
                        {LESSON_TYPES.map(({ value, label, Icon }) => (
                          <button
                            key={value}
                            type="button"
                            title={lesson.type === value ? `${label} (selected)` : label}
                            onClick={() => updateLesson(mod.id, lesson.id, { type: value })}
                            className={cn(
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <input
                          ...
                          onChange={(e) =>
                            updateLesson(mod.id, lesson.id, { durationMinutes: Number(e.target.value) })
                          }

...
import { Menu, X, GraduationCap, BookOpen, LayoutDashboard, Calendar, BarChart3, Award, Settings, Users, Layers } from "lucide-react";
...
function MobileNavLink({ label, href, icon: Icon, onClose }: { label: string; href: string; icon: LucideIcon; onClose: () => void }) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "bg-primary-muted text-primary" : "text-text-secondary hover:bg-surface hover:text-text-primary"
      )}
    >
        ...
        <div className="border-t border-border p-3">
          <div className="flex items-center gap-3 rounded-xl px-3 py-2">
            ...
            <div className="flex flex-col min-w-0">
              <span className="truncate text-sm font-medium text-text-primary">{currentUser.name}</span>
              <span className="truncate text-xs text-text-muted capitalize">{currentUser.role} · {currentUser.xp.toLocaleString()} XP</span>
            </div>
          </div>
        </div>
      </div>

...
function detectType(url: string): "vimeo" | "youtube" | "mux" | "html5" | "placeholder" {
...
export function VideoPlayer({ contentUrl, title, lessonId, courseId, onProgress }: VideoPlayerProps) {
  const type = detectType(contentUrl ?? "");
  if (type === "vimeo")
    return <VimeoPlayer url={contentUrl!} title={title} lessonId={lessonId} courseId={courseId} onProgress={onProgress} />;
  if (type === "youtube")
    return <YouTubePlayer url={contentUrl!} title={title} lessonId={lessonId} />;
  if (type === "mux")
    return <MuxPlayer playbackId={contentUrl!} title={title} lessonId={lessonId} courseId={courseId} onProgress={onProgress} />;
  if (type === "html5")
    return <Html5Player url={contentUrl!} title={title} lessonId={lessonId} courseId={courseId} onProgress={onProgress} />;
  return <PlaceholderPlayer title={title} lessonId={lessonId} courseId={courseId} onProgress={onProgress} />;
}

// ─── Vimeo ─────────────────────────────────────────────────────────────────

function VimeoPlayer({ url, title }: Omit<VideoPlayerProps, "contentUrl"> & { url: string }) {
...
function YouTubePlayer({ url, title }: Omit<VideoPlayerProps, "contentUrl"> & { url: string }) {
...
function MuxPlayer({ playbackId, title }: Omit<VideoPlayerProps, "contentUrl"> & { playbackId: string }) {
  const [loaded, setLoaded] = useState(false);
  return (
    <>
      <Script src="https://cdn.jsdelivr.net/npm/@mux/mux-player" type="module" onLoad={() => setLoaded(true)} />
      <div className="relative w-full bg-black aspect-video">
        {!loaded && (
...
function Html5Player({ url, title, lessonId, courseId, onProgress }: Omit<VideoPlayerProps, "contentUrl"> & { url: string }) {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.code === "ArrowRight") { e.preventDefault(); v.currentTime = Math.min(v.duration || 0, v.currentTime + 5); }
  ...
  return (
    <div className="group relative w-full bg-black aspect-video overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-200">
        <div className="px-4 pb-3 space-y-2">
          <input
            ...
            onChange={(e) => {
              const v = videoRef.current;
              if (v) v.currentTime = (Number(e.target.value) / 100) * v.duration;
              setPct(Number(e.target.value));
            }}
            className="w-full h-1 accent-primary cursor-pointer"
          />
          <div className="flex items-center gap-3">
            <button type="button" aria-label={playing ? "Pause" : "Play"} onClick={togglePlay} className="text-white hover:text-primary transition-colors">
            ...
            <button type="button" aria-label="Fullscreen" onClick={() => videoRef.current?.requestFullscreen()} className="text-white/70 hover:text-white transition-colors">
      ...
      {/* Centre play button */}
      {!playing && (
        <button type="button" aria-label="Play" onClick={togglePlay} className="absolute inset-0 flex items-center justify-center">
...
function PlaceholderPlayer({ title, lessonId, courseId, onProgress }: VideoPlayerProps) {
  const [playing, setPlaying]   = useState(false);
  ...
  return (
    <div className="relative bg-black aspect-video w-full max-h-[calc(100vh-14rem)] flex items-center justify-center group">
      ...
      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent px-4 pb-3 pt-8 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-3">
          <button type="button" aria-label={playing ? "Pause" : "Play"} onClick={handleToggle} className="text-white/80 hover:text-white transition-colors">

...
import { CheckCircle2, Circle, Lock, PlayCircle, Video, FileText, Presentation, Radio, Download, Clock } from "lucide-react";
import { cn, formatMinutes } from "@/lib/utils";
import { Progress } from "@/components/ui/progress";
import { isModuleUnlocked } from "@/lib/module-unlock";
import type { Module, Lesson, LearnerProgress } from "@/types";
...
export function LessonSidebar({ courseId, modules, lessons, currentLessonId, progress }: LessonSidebarProps) {
  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col border-l border-border bg-background-secondary overflow-hidden">
      ...
      <div className="border-b border-border px-4 py-3">
        <p className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">Course Content</p>
        <div className="flex items-center justify-between text-xs text-text-muted mb-1.5">
          <span>{completedIds.length} / {totalLessons} lessons</span>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        {sorted.map((mod) => {
          ...
          const modDone     = modLessons.filter((l) => completedIds.includes(l.id)).length;
          const isModDone   = modDone === modLessons.length && modLessons.length > 0;
            ...
            <div key={mod.id}>
              ...
              <div className={cn(
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="truncate text-xs font-semibold text-text-primary">{mod.title}</p>
                </div>
                <span className="ml-2 shrink-0 text-[10px] text-text-muted">{modDone}/{modLessons.length}</span>
              </div>

              {/* Lessons */}
              {unlocked && modLessons.map((lesson) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col gap-0.5 min-w-0">
                      <p className={cn("truncate text-xs font-medium", isCurrent ? "text-primary" : isDone ? "text-text-secondary" : "text-text-primary")}>
                        {lesson.title}
                      </p>
                      {lesson.durationMinutes && (
                        <span className="flex items-center gap-1 text-[10px] text-text-muted">
                          <Clock className="h-2.5 w-2.5" />{formatMinutes(lesson.durationMinutes)}
                        </span>
                      )}
                    </div>
                  </Link>

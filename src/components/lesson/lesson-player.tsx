...
import {
  ArrowLeft, ArrowRight, CheckCircle2, ChevronLeft, ChevronRight, PanelRight,
...
import type { Lesson, Quiz, LearnerProgress, Course, LearningEvent } from "@/types";
...
export function LessonPlayer({
  ...
  const [completed, setCompleted] = useState<boolean>(() => {
    const fromProgress = progress?.completedLessonIds.includes(lesson.id) ?? false;
  ...
  useEffect(() => {
    if (sidebarRef.current) {
      sidebarRef.current.style.width = sidebarOpen ? `${sidebarWRef.current}px` : "0px";
    }
  }, [sidebarOpen]);

  const completedCount = (progress?.completedLessonIds.length ?? 0) + (completed ? 0 : 1);
  ...
  function handleMarkComplete() {
    ...
    try {
      ...
      if (!stored.includes(lesson.id)) {
        localStorage.setItem(storageKey, JSON.stringify([...stored, lesson.id]));
      }
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      {showCert && (
        <CertificateEarnedModal
          ...
          onClose={() => { setShowCert(false); router.push(`/courses/${courseId}`); }}
        />
      )}
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur-sm z-10">
        <nav className="flex items-center gap-1.5 min-w-0 text-xs" aria-label="Breadcrumb">
          <Link href="/courses" className="shrink-0 text-text-muted hover:text-text-primary transition-colors">
            Courses
          </Link>
          ...
          <ChevronRight className="h-3 w-3 shrink-0 text-text-muted" />
          <span className="text-text-primary font-medium min-w-0">{lesson.title}</span>
          {completed && <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-400 ml-1" />}
        </nav>
        <button
          type="button"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto">
            ...
            {lesson.type === "live"   && <LiveSession event={linkedEvent ?? null} lessonTitle={lesson.title} />}
          </div>
          ...
          <div className="shrink-0 border-t border-border bg-background/90 px-6 py-3 backdrop-blur-sm">
            <div className="flex items-center justify-between gap-4">
              <Link
                href={prevLessonId ? `/courses/${courseId}/learn/${prevLessonId}` : `/courses/${courseId}`}
              ...
              >
                {completed ? (
                  <><CheckCircle2 className="h-4 w-4" /> {nextLessonId ? "Go to next" : "Course done!"}</>
                ) : (
                  <>{nextLessonId ? <>Complete & Next <ArrowRight className="h-4 w-4" /></> : "Complete course"}</>
                )}
              </button>
            </div>
          </div>
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <div className="prose prose-invert max-w-none space-y-4 text-sm text-text-secondary leading-relaxed">
        <p>This lesson covers the foundational concepts behind <strong className="text-text-primary">{lesson.title}</strong>. As you read through, pay attention to key principles and how they connect to what you&apos;ve already learned.</p>
        <h2 className="text-base font-semibold text-text-primary mt-6">Core Concepts</h2>
        <p>The framework introduced here builds on previous modules. The goal is not just to memorise definitions, but to understand how each concept applies in real-world design and product contexts.</p>
        <h2 className="text-base font-semibold text-text-primary mt-6">Key Takeaways</h2>
        <ul className="space-y-2 list-disc list-inside">
          ...
          <li>Think about one real example where this applies in your context</li>
        </ul>
        <p className="mt-6 rounded-xl border border-primary/20 bg-primary-muted p-4 text-xs">
          💡 <strong className="text-primary">AI Tutor tip:</strong> Click the sparkle button in the bottom right to ask your AI tutor questions about this lesson.
        </p>
      </div>
    </div>
  );
...
function SlidesContent({ lesson }: { lesson: Lesson }) {
  ...
  const slides = [
    ...
    { title: "Key Concepts", body: "The core principles and their applications in practice." },
    { title: "Examples", body: "Real-world examples that illustrate the concepts covered." },
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 py-10">
      <div className="w-full max-w-2xl">
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="text-xs text-text-muted">{lesson.title} · Slide {slide + 1} of {slides.length}</span>
        </div>
        <div className="rounded-2xl border border-border bg-surface shadow-soft-lg p-10 min-h-64 flex flex-col items-center justify-center text-center animate-fade-in">
          <h2 className="text-2xl font-bold text-text-primary mb-4">{current.title}</h2>
          ...
          <div className="flex gap-1.5">
            {slides.map((s, i) => (
              <button
                ...
                onClick={() => setSlide(i)}
                className={cn("h-1.5 rounded-full transition-all duration-200", i === slide ? "w-6 bg-primary" : "w-1.5 bg-border hover:bg-border-strong")}
              />
            ))}
          </div>
          <button
        ...
        <div className="mt-3">
          <Progress value={Math.round(((slide + 1) / slides.length) * 100)} size="xs" />
        </div>
      </div>
    </div>
  );

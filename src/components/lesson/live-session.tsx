  ...
  if (!event) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center px-6 py-16 text-center animate-fade-in">
        ...
        <h2 className="mb-2 text-lg font-bold text-text-primary">Live Session</h2>
        <p className="text-sm text-text-secondary max-w-sm">
          <span className="font-medium text-text-primary">{lessonTitle}</span> is a live session. No recording is available yet.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col">
        ...
        <div className="relative bg-black aspect-video w-full max-h-[calc(100vh-14rem)] flex items-center justify-center group">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 text-center px-8">
            ...
            <div>
              <p className="text-sm font-semibold text-white/90 mb-1">{event.title}</p>
              <p className="text-xs text-white/60">Recorded lecture · {formatMinutes(event.durationMinutes)}</p>
            </div>
          </div>
        ...
        <div className="border-t border-border bg-background-secondary px-6 py-4">
          <div className="flex flex-wrap items-center gap-4 text-xs text-text-muted">
            ...
            <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{formatMinutes(event.durationMinutes)}</span>
    ...
    <div className="flex min-h-[50vh] flex-col items-center justify-center px-6 py-16 text-center animate-fade-in">
      ...
      <div className="mb-6">
        {isLive ? (
          ...
          <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-text-muted">Upcoming Live Session</p>
        )}
        <h2 className="mt-2 text-lg font-bold text-text-primary">{event.title}</h2>
        <p className="mt-2 text-sm text-text-secondary max-w-sm mx-auto">{event.description}</p>
        <div className="mt-3 flex items-center justify-center gap-4 text-xs text-text-muted">
          {!isLive && (
            <span>{scheduledDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })} · {scheduledDate.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</span>
          )}
      ...
      {isLive && event.meetingUrl ? (
        ...
        >
          <Radio className="h-4 w-4" /> Join Now <ExternalLink className="h-3.5 w-3.5 opacity-70" />
        </a>
      ) : (
        <Link
          href="/events"

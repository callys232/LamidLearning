    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-2">{course.title}</h3>
          <div className="flex flex-wrap gap-2 text-xs text-text-muted">
            <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{formatMinutes(course.estimatedHours * 60)}</span>
            <span className="flex items-center gap-1"><BookOpen className="h-3 w-3" />{course.moduleIds.length} modules</span>
            <span className="flex items-center gap-1"><Award className="h-3 w-3" />Certificate included</span>
          </div>
        </CardContent>
      </Card>

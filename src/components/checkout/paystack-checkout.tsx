...
export function PaystackCheckout({
  ...
  function handlePay() {
    ...
    const handler = window.PaystackPop.setup({
      ...
      ref:      generateRef(),
      label:    courseTitle,
      metadata: { courseId, custom_fields: [{ display_name: "Course", variable_name: "course", value: courseTitle }] },
      ...
      callback: async (response) => {
        ...
        const res = await fetch("/api/paystack/verify", {
          ...
          body:    JSON.stringify({ reference: response.reference, courseId, userId }),
        });
    ...
    <>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          <p className="text-xs font-medium text-text-secondary">Accepted payment methods</p>
          <div className="flex flex-wrap gap-2">
            {["Visa", "Mastercard", "Bank Transfer", "USSD", "Mobile Money"].map((m) => (
              <span key={m} className="rounded-md border border-border bg-surface px-2.5 py-1 text-xs text-text-secondary">
                {m}
              </span>
            ))}
          </div>
        ...
        <p className="flex items-center justify-center gap-1.5 text-[11px] text-text-muted">
          ...
          Secured by <span className="font-semibold text-text-primary">Paystack</span> · 256-bit SSL encryption
        </p>
      </div>
    </>
  );

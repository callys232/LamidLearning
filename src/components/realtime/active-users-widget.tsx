...
interface RealtimePayload {
  activeUsers:  number;
  recentEvent:  { id: string; user: string; action: string; timestamp: string } | null;
...
export function ActiveUsersWidget() {
  ...
  const [alert,       setAlert]       = useState<{ module: string; rate: number } | null>(null);
      ...
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-sm">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            {connected
              ? <><Wifi className="h-3 w-3 text-emerald-400" /><span className="text-[10px] text-emerald-400">Live</span></>
              : <><WifiOff className="h-3 w-3 text-text-muted" /><span className="text-[10px] text-text-muted">Reconnecting…</span></>
            }
          </div>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {alert && (
          <div className="flex items-start gap-2 rounded-xl border border-yellow-500/20 bg-yellow-500/8 p-3 animate-scale-in">
            ...
            <div>
              <p className="text-xs font-semibold text-yellow-400">Drop-off alert</p>
              <p className="text-xs text-text-secondary">
                <span className="font-medium">{alert.module}</span> — {alert.rate}% of learners leaving at this module
              </p>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          {events.length === 0 ? (
            ...
            <ul className="space-y-2">
              {events.map((evt) => (
                <li key={evt.id} className="flex items-center justify-between gap-3 animate-fade-up">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="h-1.5 w-1.5 rounded-full bg-emerald-400 shrink-0" />
                    <span className="text-xs text-text-primary font-medium shrink-0">{evt.user}</span>
                    <span className="truncate text-xs text-text-secondary">{evt.action}</span>
                  </div>

...
function setCurrentUser(predicate: (u: User) => boolean): boolean {
  ...
  if (next) { currentUser = next; return true; }
  return false;
}

export const setCurrentUserById    = (id: string)    => setCurrentUser((u) => u.id === id);
export const setCurrentUserByEmail = (email: string) => setCurrentUser((u) => u.email.toLowerCase() === email.toLowerCase());
